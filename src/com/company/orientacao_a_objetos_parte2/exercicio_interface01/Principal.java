...
No momento, a corretora trabalha apenas com seguros de carros e imóveis, mas em breve serão adicionados outros tipos de contratos de seguros.
 Essa informação é muito importante, pois o software deve ter a capacidade de receber novos tipos de seguros facilmente, como por exemplo, notebook, barcos, aeronaves, etc.
...
Veja que o método fazerPropostaSeguro() recebe como parâmetro um objeto do tipo Seguravel, ou seja, pode ser um Carro ou um Imovel, mas no futuro poderia receber também uma Aeronave ou Barco,
 desde que essas classes implementem a interface Seguravel.
...
9. Quando tudo estiver funcionando, crie uma classe chamada Barco e outra chamada Notebook. As duas classes devem implementar a interface Seguravel.
 Implemente os métodos da interface e adicione algumas linhas na classe Principal para testar as novas classes.
 */
public class Principal {
    public static void main(String[] args) {
        Seguravel c1 = new Carro("Fiat", 22);
        c1.calcularValorApolice();
        c1.obterDesricao();
        Seguravel i1 = new Imovel("Casa legal", 55);
        i1.obterDesricao();
        i1.calcularValorApolice();
        Seguravel j1 = new Carro("BMW",7.50);
        CorretoraSeguro corretoraSeguro = new CorretoraSeguro();
        corretoraSeguro.realizaProposta(j1);
        Seguravel l1 = new Lancha("Lancha foda que anda na agua", 750000.50, 4000);
        l1.obterDesricao();
        l1.calcularValorApolice();
    }
}
